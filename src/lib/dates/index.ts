...
export const buildDates = (events: CalendarEvent[], month: string) => {
  ...
  const monthStart = startOfMonth(new Date(cutOffDate().getFullYear(), dateMonth, 1));
  ...
  return dateRange.map((date) => {
    ...
    return {
      ...
      isToday: formatTz(date, 'yyyy-MM-dd', { timeZone }) === formatTz(new Date(), 'yyyy-MM-dd', { timeZone }),
    };
  });
};
